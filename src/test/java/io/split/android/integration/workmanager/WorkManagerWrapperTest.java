package io.split.android.integration.workmanager;
...
public class WorkManagerWrapperTest {
    ...
    private void assertWorkSpecMatches(WorkSpec workSpec, WorkSpec expectedWorkSpec) {
        ...
        compareData(expectedWorkSpec.input, workSpec.input);
    ...
    private void compareData(Data expected, Data actual) {
        Map<String, Object> e = expected.getKeyValueMap();
        Map<String, Object> a = actual.getKeyValueMap();
        assertEquals("Input Data size differs", e.size(), a.size());
        for (Map.Entry<String, Object> entry : e.entrySet()) {
            String key = entry.getKey();
            Object ev = entry.getValue();
            assertTrue("Missing key in actual Data: " + key, a.containsKey(key));
            Object av = a.get(key);

            if ("certificatePins".equals(key) && ev instanceof String && av instanceof String) {
                assertJsonEquals((String) ev, (String) av);
            } else if (ev instanceof String[] && av instanceof String[]) {
                assertTrue("Array value differs for key: " + key, Arrays.equals((String[]) ev, (String[]) av));
            } else if (ev instanceof Integer && av instanceof Integer) {
                assertEquals("Int value differs for key: " + key, ev, av);
            } else if (ev instanceof Long && av instanceof Long) {
                assertEquals("Long value differs for key: " + key, ev, av);
            } else if (ev instanceof Boolean && av instanceof Boolean) {
                assertEquals("Boolean value differs for key: " + key, ev, av);
            } else if (ev instanceof String && av instanceof String) {
                assertEquals("String value differs for key: " + key, ev, av);
            } else {
                // Fallback for other supported Data types
                assertEquals("Value differs for key: " + key, ev, av);
            }
        }
    }
