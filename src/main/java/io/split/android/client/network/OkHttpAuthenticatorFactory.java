...
class OkHttpAuthenticatorFactory implements InternalAuthenticatorFactory<Authenticator> {

    @NonNull
    @Override
    public okhttp3.Authenticator getAuthenticator(@NonNull SplitAuthenticator splitAuthenticator) {
        return new okhttp3.Authenticator() {
            ...
            public Request authenticate(@Nullable Route route, @NonNull Response response) {
                try {
                    AuthenticatedRequest<?> authenticatedRequestResult = splitAuthenticator.authenticate(new SplitAuthenticatedRequest(response));
