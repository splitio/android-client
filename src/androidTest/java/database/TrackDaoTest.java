...
public class TrackDaoTest {
    ...
    public void insertRetrieve() throws InterruptedException {
        ...
        List<EventEntity> activeTrackEvents = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_ACTIVE);
        List<EventEntity> deletedTrackEvents = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_DELETED);
    ...
    public void insertUpdateRetrieve() throws InterruptedException {
        ...
        mRoomDb.eventDao().updateStatus(idsToSoftDelete, EventEntity.STATUS_DELETED);
        List<EventEntity> afterSoftDelete = mRoomDb.eventDao().getBy(0, EventEntity.STATUS_ACTIVE);

        mRoomDb.eventDao().delete(idsToDelete);
        List<EventEntity> afterDelete = mRoomDb.eventDao().getBy(0, EventEntity.STATUS_ACTIVE);
        List<EventEntity> softDeletedAfterDelete = mRoomDb.eventDao().getBy(0, EventEntity.STATUS_DELETED);

        mRoomDb.eventDao().deleteOutdated(timestamp + 6);
        List<EventEntity> afterAll = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_ACTIVE);
        List<EventEntity> deletedAfterAll = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_DELETED);
    ...
    public void dataIntegrity() {
        ...
        EventEntity activeEventEntity = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_ACTIVE).get(0);
        EventEntity deletedEventEntity = mRoomDb.eventDao().getBy(timestamp, EventEntity.STATUS_DELETED).get(0);
    ...
    private void performance(int count) {
        ...
        trackEventEntities = mRoomDb.eventDao().getBy(0, EventEntity.STATUS_ACTIVE);
